...
const overlay: React.CSSProperties = {
  position: 'fixed', inset: 0, zIndex: 200, background: 'rgba(11,24,41,.85)',
  display: 'flex', alignItems: 'flex-end', justifyContent: 'center', backdropFilter: 'blur(8px)',
};
const sheet: React.CSSProperties = {
  background: '#0c1a2e', borderTop: '1px solid rgba(123,200,160,.25)', borderRadius: '20px 20px 0 0',
...
const title: React.CSSProperties = {
  fontFamily: "'Playfair Display',serif", fontSize: 18, color: '#F4EDE1', marginBottom: 18,
};
const label: React.CSSProperties = {
  fontFamily: T, fontSize: 9, color: '#A8B8C8', textTransform: 'uppercase', letterSpacing: '.15em', marginBottom: 6, display: 'block',
};
const input: React.CSSProperties = {
  width: '100%', padding: '11px 14px', background: 'rgba(255,255,255,.05)', border: '1px solid rgba(123,200,160,.2)',
  borderRadius: 10, fontSize: 13, color: '#F4EDE1', fontFamily: O, outline: 'none', marginBottom: 14,
};
const primary: React.CSSProperties = {
  width: '100%', padding: 14, background: 'linear-gradient(135deg,#7BC8A0,#3D8B5E)', color: '#0B1829',
...
const secondary: React.CSSProperties = {
  ...
  borderRadius: 10, fontFamily: O, fontSize: 12, color: '#A8B8C8', cursor: 'pointer', marginTop: 8,
};
      ...
      <div style={sheet} onClick={e => e.stopPropagation()}>
        ...
        {type === 'addTask' && (
          <>
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 14 }}>
              {FAMILY.map(m => (
                <button key={m.id} onClick={() => set('assignee', m.id)} style={{ padding: '7px 12px', borderRadius: 20, fontSize: 11, cursor: 'pointer', fontFamily: O, background: form.assignee === m.id ? 'rgba(123,200,160,.15)' : 'rgba(255,255,255,.04)', color: form.assignee === m.id ? '#7BC8A0' : '#A8B8C8', border: `1px solid ${form.assignee === m.id ? 'rgba(123,200,160,.3)' : 'rgba(123,200,160,.14)'}`, display: 'inline-flex', alignItems: 'center', gap: 6 }}>
            ...
            <div style={{ display: 'flex', gap: 6, marginBottom: 14 }}>
              {(['high', 'medium', 'low'] as const).map(p => (
                <button key={p} onClick={() => set('priority', p)} style={{ flex: 1, padding: '8px 12px', borderRadius: 10, fontSize: 11, cursor: 'pointer', fontFamily: O, textTransform: 'capitalize', background: form.priority === p ? 'rgba(123,200,160,.15)' : 'rgba(255,255,255,.04)', color: form.priority === p ? '#7BC8A0' : '#A8B8C8', border: `1px solid ${form.priority === p ? 'rgba(123,200,160,.3)' : 'rgba(123,200,160,.14)'}` }}>{p}</button>
              ))}
            </div>
            <button style={primary} onClick={() => {
        ...
        {type === 'upload' && (
          <>
            ...
            <div style={{ border: '2px dashed rgba(123,200,160,.3)', borderRadius: 14, padding: 28, textAlign: 'center', marginBottom: 16 }}>
              <div style={{ fontSize: 28, marginBottom: 8 }}>↑</div>
              <div style={{ fontSize: 13, fontWeight: 600, color: '#F4EDE1' }}>Tap to upload or take a photo</div>
              <div style={{ fontSize: 11, color: '#A8B8C8', marginTop: 4 }}>PDF, JPG, PNG up to 25MB</div>
            </div>
            <label style={label}>Document Type</label>
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 14 }}>
              {['Medical', 'Insurance', 'Legal', 'Care Plan'].map(t => (
                <button key={t} onClick={() => set('type', t)} style={{ padding: '7px 12px', borderRadius: 20, fontSize: 11, cursor: 'pointer', fontFamily: O, background: form.type === t ? 'rgba(123,200,160,.15)' : 'rgba(255,255,255,.04)', color: form.type === t ? '#7BC8A0' : '#A8B8C8', border: `1px solid ${form.type === t ? 'rgba(123,200,160,.3)' : 'rgba(123,200,160,.14)'}` }}>{t}</button>
              ))}
            </div>
        ...
        {type === 'notif' && (
          <>
            ...
            <div style={{ fontSize: 13, color: '#A8B8C8', marginBottom: 18, lineHeight: 1.65 }}>
